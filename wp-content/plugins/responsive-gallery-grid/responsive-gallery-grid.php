...
//        echo '<pre>'.print_r($rgg_settings,true).'</pre>';
//        echo '<pre>'.print_r($rgg_options,true).'</pre>';
	    ...
	    if ($single_gallery_settings['lightbox'] == 'swipebox') {
		    wp_enqueue_script( 'rgg-swipebox', RGG_PLUGIN_DIR_URL . '/lib/swipebox/js/jquery.swipebox.min.js', array( 'jquery' ), RGG_VERSION );
		    $deps[] = 'rgg-swipebox';
	    }
	    if ($single_gallery_settings['lightbox'] == 'simplelightbox') {
	    ...
	    if ($single_gallery_settings['lightbox'] == 'image-above') {
            wp_enqueue_script('slick', '//cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.js', ['jquery'], RGG_VERSION);
            $deps[] = 'slick';
        }
    ...
    function rgg_register_styles() {
        ...
        wp_enqueue_style( 'rgg-swipebox', RGG_PLUGIN_DIR_URL.'/lib/swipebox/css/swipebox.min.css', array(), RGG_VERSION);
        $deps[] = 'rgg-swipebox';
