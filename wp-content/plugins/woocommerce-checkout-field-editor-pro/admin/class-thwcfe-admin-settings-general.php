...
class THWCFE_Admin_Settings_General extends THWCFE_Admin_Settings {
	...
	*-----------------------------------*/
        ...
        <form method="post" id="thwcfe_<?php echo $form_type ?>_section_form" action="">
            ...
            <div id="thwcfe-tabs-container_<?php echo $form_type ?>">
                <ul class="thpladmin-tabs-menu">
                    ...
                    <li><a class="thwcfe_tab_rules_link" href="javascript:void(0)" 
                    onclick="thwcfeOpenFormTab(this, 'thwcfe-section-tab-repeat-rules', '<?php echo $form_type ?>')">Repeat Rules</a></li>
                </ul>
	...
	private function output_section_info_form(){
        ...
        <tr>
            <?php
        	$this->render_form_field_element($this->section_form_props['name'], $this->cell_props_L);
        	$this->render_form_field_element($this->section_form_props['position'], $this->cell_props_R1);
        	?> 
        </tr>  
        <tr>
        	<?php
        	$this->render_form_field_element($this->section_form_props['cssclass'], $this->cell_props_L);
        	$this->render_form_field_element($this->section_form_props['order'], $this->cell_props_R1);
        	?>         
        </tr> 
        <?php
	}
	...
	private function output_title_form($show_subtitle = false){
        ...
        <tr>
        	...
        	<td colspan="4">
        	<?php
        	$this->render_form_field_element($this->section_form_props['show_title'], $this->cell_props_CB, false);
			$this->render_form_field_element($this->section_form_props['show_title_my_account'], $this->cell_props_CB, false);
			?>
            </td>
        </tr> 
        ...
        <tr>
        	<?php
        	$this->render_form_field_element($this->section_form_props['title'], $this->cell_props_L);
        	$this->render_form_field_element($this->section_form_props['title_type'], $this->cell_props_R1);
        	?>
        </tr>  
        <tr>
        	<?php
        	$this->render_form_field_element($this->section_form_props['title_color'], $this->cell_props_CP);
        	$this->render_form_field_element($this->section_form_props['title_class'], $this->cell_props_R1);
        	?>
        </tr>
        ...
        <tr>
        	<?php
        	$this->render_form_field_element($this->section_form_props['subtitle'], $this->cell_props_L);
        	$this->render_form_field_element($this->section_form_props['subtitle_type'], $this->cell_props_R);
        	?>
        </tr>  
        <tr>
        <?php
        	$this->render_form_field_element($this->section_form_props['subtitle_color'], $this->cell_props_CP);
        	$this->render_form_field_element($this->section_form_props['subtitle_class'], $this->cell_props_R);
        	?>
        </tr>
        <?php
		}
	...
	private function output_h_separator($show_line = true){
		...
		?>
		<tr><td colspan="6" <?php echo $style; ?> ></td></tr> 
        <?php
	}
	...
	*---------------------------------------------*/
	...
	private function output_content(){
        ...
        <div class="wrap woocommerce"><div class="icon32 icon32-attributes" id="icon-woocommerce"><br /></div>                
            ...
            <table id="thwcfe_checkout_fields" class="wc_gateways widefat thwcfe_fields_table" cellspacing="0">
                <thead>
	...
	*******************************************************************************/	
	/*public function woo_admin_order_data_after_order_details($order){	
		...
		$dis_sections = WCFE_Checkout_Fields_Utils::get_disabled_sections($order_id);
		$dis_fields = WCFE_Checkout_Fields_Utils::get_disabled_fields($order_id);

		foreach($sections as $sname => $section){
			if(!in_array($sname, $dis_sections) && $sname != 'billing' && $sname != 'shipping'){
				...
				//}
				$this->display_section_in_admin_order($order, $order_id, $sname, $section, $dis_fields, '<p>&nbsp;</p>');

				if($sname != 'additional'){
					$this->may_display_repeat_sections_in_admin_order($order, $order_id, $sname, $section, $dis_fields, $rsnames);
				}
			}			
		}		
	}
	...
	public function woo_admin_order_data_after_billing_address($order){
		...
		if(THWCFE_Utils_Section::is_valid_section($section)){
			...
			$html = $this->display_fields_in_admin_order($order, $fields, $dis_fields, '');
	...
	public function display_section_in_admin_order($order, $order_id, $key, $section, $dis_fields, $html_prefix=''){
		$cart_info = THWCFE_Utils::get_order_summary($order);
	...
	public function may_display_repeat_sections_in_admin_order($order, $order_id, $key, $section, $dis_fields, $rsnames){
		...
		if(is_array($rsections)){
			foreach($rsections as $rskey => $rsection) {
				$this->display_section_in_admin_order($order, $order_id, $rskey, $rsection, $dis_fields);
			}
		}
	}
	...
	public function display_fields_in_admin_order($order, $fields, $dis_fields, $field_name_prefix = ''){
		$html = '';
			...
			if(!empty($cfields)){
				...
				<table class="form-table thwcfe-user-profile" id="<?php echo esc_attr( 'thwcfe_fieldset-' . $sname ); ?>">
					...
					foreach($cfields as $key => $field){
						...
						$field_html  = $this->form_field_file_html($key, $field, $value);
						$field_html .= $this->prepare_file_preview_html($value);
						?>
						<tr class="thwcfe-input-field-wrapper">
							<th>
								<label for="<?php echo esc_attr($key); ?>"><?php echo esc_html(THWCFE_i18n::t($title)); ?></label>
							</th>
							<td>
								<?php echo $field_html; ?>
								<p class="description"><?php echo wp_kses_post(THWCFE_i18n::t($desc)); ?></p>
							</td>
						</tr>
						<?php
					}
	...
	private function form_field_file_html( $key, $args, $value ) {
		$fname = esc_attr($key);
		...
		$field .= '<input type="hidden" class="thwcfe-checkout-file-value input-text '.esc_attr(implode(' ', $hinput_class)) .'" name="'. $fname .'" id="'. $fname .'" value="'. $value_json .'" />'; 
		
		$field .= '<input type="'. esc_attr( $args['type'] ) .'" class="thwcfe-checkout-file '.esc_attr(implode(' ', $input_class)) .'" name="'. $fname .'_file" '; 
		$field .= 'id="'. $fname .'_file" placeholder="' . esc_attr($args['placeholder']) . '" value="' . esc_attr($value) . '" '.$custom_file_field_attr.' />';
		
		return $field;
	}
	private function prepare_file_preview_html($value, $hyperlink=true){
		$prev_html = '';
	...
	*------------------------------------------*/
		...
		<form method="post" id="thwcfe_<?php echo $form_type ?>_field_form" action="">
        	...
        	<div id="thwcfe-tabs-container_<?php echo $form_type ?>">
                <ul class="thpladmin-tabs-menu">
                    ...
                    <li><a class="thwcfe_tab_rules_link" href="javascript:void(0)" 
                    onclick="thwcfeOpenFormTab(this, 'thwcfe-tab-display-rules', '<?php echo $form_type ?>')">Display Rules</a></li>
                    <li><a class="thwcfe_tab_rules_link" href="javascript:void(0)" 
                    onclick="thwcfeOpenFormTab(this, 'thwcfe-tab-repeat-rules', '<?php echo $form_type ?>')">Repeat Rules</a></li>
                </ul>
	...
	private function render_field_form_fragment_category_list(){		
        ...
        <select multiple="multiple" name="i_rule_operand" data-placeholder="Click to select categories" class="thwcfe-enhanced-multi-select thwcfe-operand thwcfe-category-select" style="width:200px;" value="">
			<?php 	
